...
const BookmarkLayout = (function () {
    ...
    function renderPostCard(b) {
        ...
        const title = b.postTitle ? escapeHtml(b.postTitle) : (isDeleted ? "삭제된 게시물" : "");
        const content = b.postContent ? escapeHtml(b.postContent) : "";
        const nickname = b.memberNickname ? escapeHtml(b.memberNickname) : (b.memberHandle ? escapeHtml(b.memberHandle.replace(/^@/, "")) : "사용자");
        ...
        const mediaHtml = postFiles.length > 0
            ...
            : "";
        return `<article class="bookmark-post${isDeleted ? " bookmark-post--deleted" : ""}" data-post-id="${b.postId}" data-bookmark-id="${b.id}" data-post-member-id="${b.postMemberId || ""}">
            ${avatarHtml}
